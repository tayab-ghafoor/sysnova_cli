...
# P0-3 FIX: Guard readline import for Windows/minimal containers.
try:
...
class CommandHistory:
    ...
    def _load_history(self) -> None:
        """Load persisted history from the data directory."""
        try:
            from system_manager_cli.config.config import Config  # type: ignore[import]

            history_file = Path(Config.DATA_DIR) / ".cli_history"
    ...
    def _save_history(self) -> None:
        """Persist history to the data directory."""
        try:
            from system_manager_cli.config.config import Config  # type: ignore[import]
...
class CommandAutocomplete:
    ...
    def autocomplete_all(cls, partial: str) -> list[str]:
        ...
        if partial_lower.isdigit():
            matches.extend([o for o in ["0", "1", "2", "3", "4", "5", "6", "7"] if o.startswith(partial_lower)])
...
def setup_readline() -> None:
    ...
    try:
        ...
        # macOS libedit compatibility check
